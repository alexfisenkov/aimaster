...
- Free-form Telegram messages enter a durable local inbox and are processed by a
...
2. Enter the BotFather token in a hidden terminal prompt; send `/start` to pair.
3. Run `python3 scripts/creator_studio_telegram.py run --workspace WS`.
4. Telegram shows projects and a Mini App button. `/open PROJECT` selects the
   same project; free-form text is queued for the local agent bridge.
