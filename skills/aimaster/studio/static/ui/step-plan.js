  ...
  button.addEventListener("click", () => {
    requestAgentPrompt({
      ...
      prompt: group.kind === "video"
        ? `Открой проект «${model.projectId}». Используй видеореференс только из файла, который я прикреплю. Сначала проверь видео и найди подходящий существующий placeholder; если он есть — обнови его, не создавай дубликат. Спроси usage: reference, motion, continue или edit. Зарегистрируй файл как video_reference и создай/обнови kind video с source=upload штатными командами Creator Studio. После записи сообщи точный reference_id VID_NN и проверь asset_id, source=upload и отсутствие активной pos:ref позиции. Не предлагай повторно генерировать приложенный материал.`,
        : `Открой проект «${model.projectId}». Я хочу добавить ${group.requestLabel} в референсы проекта. Спроси, прикреплю ли я готовый файл или нужно сгенерировать референс. Если файл уже приложен, считай выбором upload: сначала проверь вложение, найди подходящий существующий placeholder либо предложи понятное название, зарегистрируй файл и создай/обнови референс с source=upload. Не предлагай генерировать уже приложенный материал. Если выбран generate на image_plan, создай reference source=generate и его prompt, но не вызывай provider: внешний запуск и collection разрешены только когда image_results станет текущей стадией. После записи сообщи reference_id, required stage и следующий допустимый шаг.`,
