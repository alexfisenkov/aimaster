...
дашборда (план Б, «Контракт для плана Б», п. 2). Отвечает и без движка, и
когда движок не смог прочитать монтаж (`model_error`): экран «Сборка» должен
показать версии и путь к файлу, даже если схема слоёв сейчас недоступна."""
...
def _base(ctx: ProjectContext, engine: Engine | None, reason: str) -> dict:
    ...
    return {
        ...
        "model_error": None, "stale_clips": [], "desk": {"state": "closed"},
        "paths": {"current": str(ctx.paths.current), "output": None},
    }


def _stale(ctx: ProjectContext) -> list[dict]:
    ...
    return [{key: item.get(key) for key in STALE_KEYS} for item in items]
...
def _model_part(ctx: ProjectContext, engine: Engine, current_version, runner) -> dict:
    ...
    current = next((meta for meta in list_versions(paths) if meta.version == current_version), None)
    digest = model_hash(model)
    return {"model_hash": digest, "duration": model.duration, "layers": layers_view(model),
            "unrendered_changes": has_unrendered_changes(digest, current)}
