...
проекты, медиа, твои инструкции и библиотека персонажей, голосов и референсов.
При открытии нового окна терминала путь нужно ввести снова. Не выбирай
временную папку: после перезагрузки она может исчезнуть.

Раскладку папки создаёт команда `workspace init` — вручную папки заводить не
нужно. Её можно запускать повторно: существующие файлы она не трогает.
...
subprocess.run(
    [sys.executable, "scripts/creator_studio.py", "workspace", "init", str(canonical)],
    check=True,
)
state = canonical / "projects" / "first-video" / "state.json"
if state.is_file():
...
После этого в папке появятся `projects/`, `media/`, `instructions/`,
`library/` и файл `README.md` с описанием раскладки.

Если папка уже использовалась и в ней есть проекты, один раз собери их
персонажей, голоса и референсы в общую библиотеку:
...
посторонние чаты игнорируются. В этой версии в Telegram поддерживаются
текстовые сообщения и команды; медиа-вложения будут добавлены отдельным этапом.

### Как Mini App попадает в Telegram

По умолчанию, если установлен `cloudflared`, transport поднимает временный
Quick Tunnel (`--protocol http2`) и добавляет кнопку «AI Мастерская».
Такой туннель может умереть в любой момент, поэтому transport следит за ним
на каждом шаге опроса: замечает смерть, поднимает туннель заново с
нарастающей паузой между попытками, проверяет новый адрес и публикует его
снова. Пока рабочего адреса нет, кнопка снимается: web app-кнопка исчезает
из сообщений, а кнопка меню чата возвращается к обычной. Текстовые команды
и локальный bridge продолжают работать всегда.

Если у тебя уже настроен свой Cloudflare Tunnel, transport запускает
временный туннель с пустым конфигом, чтобы твои правила ingress из
`~/.cloudflared/config.yml` его не перехватывали: иначе catch-all вида
`service: http_status:404` отвечает на запросы к имени временного туннеля
и до локального шлюза они не доходят. Твой конфиг при этом не меняется.

Имя нового туннеля расходится по DNS несколько секунд, поэтому первая
проверка идёт с задержкой: ранний запрос получает отрицательный ответ,
который резолвер потом держит в кэше минутами. Если имя так и не ответило,
transport пробует поднять туннель заново; после нескольких таких попыток
кнопка публикуется без подтверждения — с отметкой в консоли, почему её не
удалось проверить. Без кнопки тебя не оставят.

### Постоянный адрес вместо временного туннеля (необязательно)

Если хочешь постоянный адрес на своём домене (например, named tunnel
Cloudflare), укажи его — и временный туннель подниматься не будет вовсе:

```zsh
export AIMASTER_MINI_APP_PUBLIC_URL="https://studio.example.com"
export AIMASTER_MINI_APP_PORT="8788"
python3 scripts/creator_studio_telegram.py run --workspace "$CREATOR_WORKSPACE"
```
...
- `AIMASTER_MINI_APP_PORT` — фиксированный порт локального шлюза,
  1024–65535. Без этой переменной порт выбирается свободный, случайный.
- Свой туннель настраиваешь и запускаешь сам; он должен вести на
  `http://127.0.0.1:<порт>` того же компьютера. Transport его не поднимает,
...
по собственному списку инструментов, скриптом `scripts/detect_tools.py` и
бесплатным пробным вызовом — и только после этого скажет, чего не хватает.
Затем покажет доступные модели и поможет выбрать подходящую. После этого одно разрешённое
...
Это режим «с уточнениями» (`guided`). В режиме «автопилот» (`autopilot`) агент
показывает короткий бриф с длительностью, форматом и референсами; после твоего
«да» он больше ничего не спрашивает до готового ролика: сам выбирает
инструкции, модель, варианты и одобряет этапы. При включении автопилота агент
один раз покажет предупреждение «Автопилот тратит кредиты без подтверждения» —
генерации в этом режиме списывают кредиты без отдельного разрешения и без
лимита. Точные команды описаны в [справочнике для агента](creator-studio.md).
