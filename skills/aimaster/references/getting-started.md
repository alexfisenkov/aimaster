...
ролик. Страницу проекта в браузере и Telegram можно подключить позже. Работать
только в чате тоже можно.
...
## 4. Открой страницу проекта в браузере (необязательно)

Если хочется видеть материалы проекта в браузере, запусти из той же папки:

```zsh
python3 scripts/creator_studio.py serve "$CREATOR_WORKSPACE" --port 0
```

Открой в браузере единственный напечатанный адрес
`http://127.0.0.1:<номер-порта>`. Страница работает, пока открыт этот процесс;
для остановки нажми `Ctrl-C`. На странице можно смотреть материалы и вносить
правки. Для создания новых материалов нужен работающий агент в чате.

## 5. Подключи Telegram (необязательно, только для владельца)

Для обычной работы в чате или браузере Telegram не нужен. Если он нужен,
владелец сам создаёт отдельного бота через официальный @BotFather и получает
токен. Токен нельзя отправлять в чат, вставлять в примеры или сохранять в
историю команд.
...
Проверь напечатанное число глазами, затем передай ID программе. Не подставляй
...
Напиши агенту идею ролика. Он уточнит аудиторию и ограничения, подготовит
...
Сначала выбери в чате сервис и согласуй лимит. После этого один разрешённый
клик на создание или перегенерацию означает один запуск — повторно спрашивать
