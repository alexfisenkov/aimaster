...
## Команда Python и операционная система

Навык работает на macOS, Linux и Windows (нативно, без WSL). Во всех командах
этого навыка и его справочников `python3` означает `python_cmd` — команду,
которую напечатал установщик `scripts/install.py` для этой машины: обычно
`python3` на macOS/Linux и `py -3` на Windows. Если `python_cmd` неизвестен,
запусти из папки навыка `python3 scripts/install.py --print-python-cmd`
(на Windows — `py -3 scripts/install.py --print-python-cmd`): команда ничего не
устанавливает и печатает только `python_cmd`. Пользователя про версии не
спрашивай. Пути с пробелами и кириллицей бери в кавычки. Переменные окружения
в примерах записаны для POSIX-оболочки (`export NAME=value`, `"$NAME"`); в
PowerShell это `$env:NAME = "value"` и `$env:NAME`. Примеры с heredoc
(`python3 - <<'PY'`) в PowerShell не работают: сохрани код во временный `.py`
файл вне workspace и запусти его через `python_cmd`.
...
- **Дашборд:** ничего устанавливать не нужно, если доступен Python 3.11+.
  ...
  `Ctrl-C`.
- **Telegram (по желанию; живой запуск проверен на macOS):**
  `python3 scripts/creator_studio_telegram.py setup-ui` открывает локальное окно
  для токена и сохраняет его в защищённом хранилище ОС (на macOS — Keychain);
  затем `run`
  ...
  На Windows и Linux живой запуск Telegram ещё не проверялся: если что-то не
  работает, честно скажи об этом и продолжай работу в чате и дашборде.
