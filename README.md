...
Версия `2026.09.21.4` — предварительный публичный выпуск. Установка из GitHub ещё
...
Для macOS Telegram теперь может быть дополнительным транспортом того же
workspace: локальный controller принимает сообщения, Codex bridge обрабатывает
их на включённом компьютере, а Mini App показывает тот же дашборд. Windows пока
